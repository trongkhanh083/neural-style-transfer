...
`train.py` trains networks that can transfer styles from artwork into images.
...
## inference.py
`inference.py` evaluates trained networks given a checkpoint directory. If evaluating images from a directory, every image in the directory must have the same dimensions.
